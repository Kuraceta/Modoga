...
public class ReflectionManager {
	...
	public ReflectionManager() {
		try {
			this.commandMap = ((SimpleCommandMap) Bukkit.getServer().getClass()
					.getDeclaredMethod("getCommandMap", new Class[0]).invoke(Bukkit.getServer(), new Object[0]));
			Object obj = Bukkit.getServer().getClass().getDeclaredMethod("getServer", new Class[0])
					.invoke(Bukkit.getServer(), new Object[0]);
			this.propertyManager = obj.getClass().getDeclaredMethod("getPropertyManager", new Class[0]).invoke(obj,
					new Object[0]);
	...
	public Object getPropertiesConfig(String name, Object obj) {
		try {
			Properties properties = (Properties) this.propertyManager.getClass().getDeclaredField("properties")
	...
	public void removeArrows(Player player) {
		try {
			...
			handle.invoke(player, new Object[0]).getClass().getMethod(methodName, new Class[] { Integer.TYPE })
					.invoke(handle.invoke(player, new Object[0]), new Object[] { Integer.valueOf(0) });
	...
	public void savePropertiesConfig() {
		try {
			this.propertyManager.getClass().getMethod("savePropertiesFile", new Class[0]).invoke(this.propertyManager,
					new Object[0]);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void sendChunk(Player p, int x, int z) {
		try {
			...
			Constructor con = Class.forName(this.currentVersion + ".ChunkCoordIntPair")
	...
	public Object setPropertiesConfig(String name, Object obj) {
		try {
			return this.propertyManager.getClass().getMethod("a", new Class[] { String.class, Object.class })
					.invoke(this.propertyManager, new Object[] { name, obj });
		} catch (Exception e) {
			e.printStackTrace();
		}
		return obj;
